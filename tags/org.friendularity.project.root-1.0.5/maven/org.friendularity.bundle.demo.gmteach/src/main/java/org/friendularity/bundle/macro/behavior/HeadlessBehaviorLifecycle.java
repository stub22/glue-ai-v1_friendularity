 ...
 * @author Ryan Biggs
 */
public class HeadlessBehaviorLifecycle extends AbstractLifecycleProvider<MasterDemo, MasterDemo> {

	private final static Logger theLogger = LoggerFactory.getLogger(HeadlessBehaviorLifecycle.class);
	private final static String queryEmitterId = "queryInterface";
	private BundleContext myContext;

	public HeadlessBehaviorLifecycle(BundleContext context) {
	...
	private void startCallbackMaps(BundleContext context,
			String behaviorPanelId, String adminPanelId) {
		new OSGiComponent(context, new SimpleLifecycle(
				new ActionCallbackMapImpl(), ActionCallbackMap.class.getName(),
				props(adminPanelId))).start();
		new OSGiComponent(context, new SimpleLifecycle(
				new ActionCallbackMapImpl(), ActionCallbackMap.class.getName(),
				props(behaviorPanelId))).start();
	}
