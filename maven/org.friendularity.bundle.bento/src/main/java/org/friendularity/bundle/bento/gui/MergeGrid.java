...
public class MergeGrid extends JPanel {
	...
	void attemptDeleteColumn(int col) {
		...
		HashMap<Point, MergeGridEntry> newCells = new HashMap<>();
		...
		for (Iterator<Point> i = myCells.keySet().iterator(); i.hasNext(); ) {
			...
			// this should be all
				...
				LoggerFactory.getLogger(MergeGrid.class.getName()).error("unhandled case deleting column "
						+ col + " from mge.col = " + mge.col + " mge.colsize = " + mge.colsize);
			}
		}
	...
	void attemptDeleteRow(int row) {
		...
		for (Iterator<Point> i = myCells.keySet().iterator(); i.hasNext(); ) {
			...
			// this should be all
				...
				LoggerFactory.getLogger(MergeGrid.class.getName()).error(
