...
public class CameraViewer extends BentoPlugin implements ImageStreamConsumer {
	...
	public void setConsumedImage(ImageStreamImage img) {
		try {
			...
			LoggerFactory.getLogger(CameraViewer.class.getName()).error("CameraViewer viewing wrong image type");
		}
	...
	/**
	 * Lay out an image in the given area so it shows the best location The best location is the
	 * size of the layout area or smaller, the right aspect ratio, and a small numbered divisor or
	 * multiplicand of the original size, and centered in gthe layout area
	 *
	...
	public void sourceIsEnding() {
		try {
			...
			LoggerFactory.getLogger(CameraViewer.class).error(ex.getMessage(), ex);
		}
	}
