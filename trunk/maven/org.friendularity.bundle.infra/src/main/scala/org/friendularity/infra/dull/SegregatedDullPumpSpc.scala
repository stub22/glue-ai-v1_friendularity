...
import org.friendularity.infra.cpump.{BoundaryTellerFinder, MutaBoundedCPumpCtxImpl}
...
class SegregatedBoundedDullPumpCtx extends DullPumpCtx with MutaBoundedCPumpCtxImpl {
	// Fullfill the contract left hanging in MutaBounded, conditioned on segregation assumption.
	override def getBoundaryTellerFinder_opt : Option[BoundaryTellerFinder] = Some(this)
}

// Keeps an explicit map so we can re-find a context in case of actor replacement, or intentional mutable state sharing.
