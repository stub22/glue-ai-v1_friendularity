...
import org.friendularity.bind.webber.ask.AskAskDotCom;
import org.friendularity.bind.webber.cogbot.CogbotCommunicator;
import org.friendularity.webber.services.GenRespWithConf;
import org.friendularity.webber.config.MeneConfig;
import org.friendularity.bind.webber.fallback.FallbackService;
import org.friendularity.bind.webber.jmx.CharacterEngineInterface;
import org.friendularity.bind.webber.jmx.JMXInterface;
import org.friendularity.bind.webber.jmx.JMXListener;
import org.friendularity.bind.webber.rss.FeedSelector;
import org.friendularity.bind.webber.rss.RSSConfig;
import org.friendularity.webber.services.BatchServiceRequest;
import org.friendularity.webber.services.INexusService;
import org.friendularity.webber.utils.MeneLogger;
...
public class Communicator implements CommunicatorMBean{
	...
	public static MenePanel myFrame;
	...
	public Communicator(){
        ...
        AskAskDotCom.myConfig = myConfig;
        myServices = new ArrayList<INexusService>();
        myServices.add(new AskAskDotCom());
        //myServices.add(new ElbotCommunicator(myConfig));
		myServices.add(new CogbotCommunicator(myConfig));
		myServices.add(new FeedSelector(RSSConfig.getConfig()));
		myServices.add(new FallbackService());
    ...
    public void startListening(){
        JMXListener.createAndRegister(this);
        try {
	...
	public void communicate(String input, boolean toConvoid){
        ...
        if(toConvoid){
			if(resp.getResponse().equals("FALLBACK")){
				JMXInterface.sendThoughtToConvoid("T_FALLBACK", 10);
			}else{
				sendResponse(resp);
			}
		}
    ...
    private void sendResponse(GenRespWithConf resp){
        JMXInterface.sendResponseToConvoid("SAY",resp.getResponse());
    }
