...
import org.friendularity.bundle.macro.jvision.CCRK_DemoMidiCommandMapper;
...
import org.friendularity.gmteach.midi.CogcharMidiInputTestMain;
...
public class LikeSuperActivator extends CommonActivator implements BundleListener, ServiceListener {
	...
	WorldEstimateRenderModule werm0 = new WorldEstimateRenderModule();
	EstimateVisualizer<?> eViz;
	protected MathGateUnscripted mg;
	protected WorldEstimate we;
	protected CCRK_DemoMidiCommandMapper myMidiMapper = new CCRK_DemoMidiCommandMapper();
	...
	public void registerServices(final BundleContext context0) {
		...
		addMacroServiceButton("attachVWorldRenderModule", new Runnable() {
			@Override public void run() {
				...
				WorldEstimateRenderModule werm = ensureWerm();
		...
		*/
		...
		addMacroServiceButton("launchBehaviorLifecycles", new Runnable() {
			@Override public void run() {
	...
	protected WorldEstimateRenderModule ensureWerm() {
		synchronized (pumaBootLock) {
			if (werm0 == null) {
				// Hey, let's get some fused-sensor-data visualization going too, while we're at it!
				werm0 = new WorldEstimateRenderModule();
				// Enable/Disable this texture flow based on whether we are launching JVision or not.
				// Should be a dont-care whether this happens before/after   startVisionMonitors() below.
				// TODO:  Re-verify in detail.
